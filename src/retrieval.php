...
function hermes_build_index(string $text, string $version): array
{
    ...
    foreach (hermes_sections($text) as $section) {
        ...
        // Wiederholte Registerverweise sind gewöhnlich kürzer als der eigentliche Abschnitt.
        if (!isset($chapters[$chapter]) || mb_strlen($section['text']) > mb_strlen($chapters[$chapter]['text'])) $chapters[$chapter] = $section;
    }
    $passages = [];
    foreach ($chapters as $section) {
        ...
        while ($offset < $length) {
            $end = min($offset + 2800, $length);
            if ($end < $length) {
                $candidate = mb_substr($body, $offset, $end - $offset);
                $break = mb_strrpos($candidate, "\n");
                if ($break !== false && $break > 1600) $end = $offset + $break;
            }
    ...
    return ['format' => 1, 'version' => $version, 'source_sha256' => hash('sha256', $text), 'passages' => $passages];
}
...
function hermes_load_index(string $version, array $release): array
{
    ...
    if (!is_array($index) || ($index['format'] ?? null) !== 1 || ($index['version'] ?? '') !== $version || !in_array($index['source_sha256'] ?? '', $hashes, true) || empty($index['passages'])) {
